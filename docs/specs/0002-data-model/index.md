...
| `canonical_id` | str | yes | The held item's own id, verbatim or derived, **not** the collapsed signature. Without it a queue row cannot say which item it is: under AC-11's signature every derived entity of one type on one line looks alike, which is exactly what a reviewer needs to tell apart. |
...
| `reasons` | list[reason] | yes | Every reason the item was held. A reason is a name plus an optional payload, not a bare word, because `endpoint_not_accepted` must carry the `canonical_id` of the entity the link is waiting on. The other three (`runs_disagree`, `known_trap_flag`, `unclassified_type`) carry no payload. |
| `model`, `queued_at` | str, str | yes | Provenance, as the run artifact records it. |

Rows are ordered by lowest located offset within a unit, so the queue reads in document order and two runs of the same extraction produce the same file.

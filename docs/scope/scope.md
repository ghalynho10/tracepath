...
**The second question: does a worked example make the labelled reference path round trip.** Not a candidate cause of the disagreement rate, a separate, concrete defect experiment 0004 already isolated: nothing in `SYSTEM_PROMPT` positively instructs the model to set `label` on an entity, only on a reference naming one. Per the engineer, not independently verified in this repo (no worked examples file exists here yet): six worked examples already drafted for this feature demonstrate entity labelling, among them `0006`'s Happy path and one about a font licence, so the fix (an explicit entity label instruction) and its test (those examples) arrive together rather than needing separate authorship.

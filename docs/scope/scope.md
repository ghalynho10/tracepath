...
- [ ] Build it: `/develop data model` · reopened 2026-09-23: spec 0002 build plan tasks 16 and 17, added by the same day's AC-7 `label` amendment; the code and its tests are built, the milestone below is what is still owed
  ...
  - [ ] A `label` on a reference endpoint: the comparison signature gains it (AC-11e), `ReferenceEndpoint` and `resolve_endpoints()` gain the label match path with the held entity, struck exclusion, and id precedence rules (AC-7, AC-10, AC-11c) · spec 0002 build plan tasks 16 and 17 · built and unit tested 2026-09-23, code in `src/tracepath/extract/schema.py`, `src/tracepath/extract/compare.py`, `src/tracepath/resolve/endpoints.py`, `src/tracepath/pipeline.py`; rebuilding `artifacts/review-queue.json` under the new signature moved accepted entities 71 to 67 and queue rows 403 to 436, all from entity labels that genuinely varied across runs. **Still owed**: a real run against spec 0001's `## Binding rules` section and 0008's `Preamble`, gated behind a cost estimate and a go ahead. The prompt's rule text was updated for `label`; a worked example was deliberately not added, flagged for feature 12 instead of preempting its own undecided question
